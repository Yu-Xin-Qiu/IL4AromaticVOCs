...
def evaluate_dataset(model, dataset, fold,shuffle,name, save=False):
    ...
    with torch.no_grad():
        for datas, label,smiles,t in loader:
    ...
    if save:
        df = pd.DataFrame({'Experimental Value': y, 'Predicted Value': pred, 'SMILES': smi, 'T': T})
...
def cross_validation(dataset,config,df,num_folds):
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(indices)):
        ...
        if config['Load pretrained model']== True:
            ...
            if config['task']== 'Toluene':
                state_dict = torch.load('model_weight/COSMO_toluene_pretrain.pth', map_location=device)
            elif config['task']== 'Styren':
                state_dict = torch.load('model_weight/EXP_toluene_pretrain.pth', map_location=device)
                # state_dict = torch.load('model_weight/COSMO_styrene_pretrain.pthh', map_location=device)
...
if __name__ == '__main__':
    ...
    if config['task']== 'Toluene':
        df = pd.read_csv("data/toluene/Clean_toluene.csv")
    elif config['task']== 'Styren':
        df = pd.read_csv("data/styrene/Styren_data.csv")
    else:
